...
-- din anii trecuți, sau dintr-o culegere. Independentă de `chapter_id` — o grilă
-- dintr-un subiect oficial ține în continuare de un capitol real, ca elevul să
-- poată filtra simultan pe materie și pe sursă (vezi CLAUDE.md, secțiunea despre
-- pornirea unei sesiuni).
create type question_sursa as enum ('materie', 'subiect_oficial', 'culegere');

alter table questions add column sursa question_sursa not null default 'materie';
-- Anul subiectului, doar la `subiect_oficial`; nefolosit altfel.
alter table questions add column an smallint check (an is null or an between 2015 and 2100);

-- Aceeași funcție ca în migrarea 0006, extinsă să citească `sursa` și `an` din payload.
...
begin
  ...
  insert into public.questions (id, chapter_id, tip, status, text, enunturi, correct, expl, src, sursa, an, created_by)
  values (
    ...
    v_sursa::public.question_sursa, v_an, auth.uid()
  )
