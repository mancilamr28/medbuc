...
const capIdDinCheie = (key: string, taxonomie: Taxonomie): ChapterId | null => {
  if (!key.startsWith(NOTE_PREFIX)) return null;
  const id = key.slice(NOTE_PREFIX.length);
  return taxonomie.esteCapitol(id) ? id : null;
};
 ...
 * nu mai există în taxonomie e exclusă, nu doar neordonată.
 */
export function capitoleCuNotita(
  ...
  for (const key of keys) {
    const capId = capIdDinCheie(key, taxonomie);
    if (capId && areTextNegol(getValue(key))) gasite.add(capId);
  }
  return taxonomie.capitole.map((c) => c.id).filter((id) => gasite.has(id));
}

/** Citește direct din `localStorage`. Rulează o singură dată, la montarea ecranului. */
export function citesteCapitoleCuNotita(taxonomie: Taxonomie = TAXONOMIE_GOALA): ChapterId[] {
  try {
    const keys: string[] = [];
    ...
    return capitoleCuNotita(keys, (k) => localStorage.getItem(k), taxonomie);
  } catch {
