...
Selection moved off the client. The global content load is now only `id` + `chapter_id`; it neither downloads the library text nor any answer. The final column lock shipped only after that client and its admin RPCs were live: table-level `select` was revoked, then only the safe columns were granted back to `authenticated`. Preserve this order in future changes — a new direct browser read must use an already-granted safe column or ship through an RPC before a grant changes.
...
- **Access is the most restrictive layer, not one flag.** A question is eligible only when both `questions.acces` and its collection's `colectii.acces` allow the current account; hiding or withdrawing the collection removes it too. The same rule now lives in `questions_citire`, so the safe catalogue cannot advertise premium questions to a free account and then have the generator refuse them later.
...
**The database also blocks the direct route.** RLS filters rows, not columns, so migration `20260828181032_blocarea_raspunsurilor` first revokes table-level `select` from `public`/`anon`/`authenticated`, then grants back an explicit list of safe columns. `correct`, `expl` and `question_options.why` are absent from that list; `select *` consequently fails too. Do not restore table-level `select` on either content table — it silently overrides every column restriction. Administrators use `citeste_grila_admin`/`exporta_grile_admin`; students receive answers only from the solving RPCs after the server permits them.
...
- **`citeste_grila_admin` is the protected admin route.** A column grant is per database role, but "admin" is an application role (`profiles.role`), so there is no role to grant to. The `security definer` function verifies `private.is_admin()` and is the reason the editor still works after the column lock.
...
- **The wizard's steps are derived, not a fixed page count.** `pasiVizibili()` drops the content step when the mode picks its own questions, or when there is neither a useful chapter choice nor a collection to choose. Collections are a second, composable axis beside subjects and chapters; a premium collection stays visible but disabled with its reason. Per-chapter numbers come from the safe catalogue; the authoritative filtered total under the card is `numara_candidati`, debounced, with out-of-order responses discarded.
...
- **`nivel_acces` / `questions.acces` / `colectii.acces` / `profiles.abonament_pana` are the entitlement boundary.** Everything still defaults to `liber`, so existing content stays open, but a row marked `premium` is now enforced by RLS and by the generation engine. A collection's level applies to every question in it and to predefined papers linked to it. The UI reads `abonament_pana` only to explain and disable choices early; that is presentation, never authorization — the database repeats every check.
